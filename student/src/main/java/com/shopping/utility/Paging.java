...
public class Paging {
	...
	public Paging(String _pageNumber, String _pageSize, int totalCount, String url, String mode, String keyword, boolean isGrid) {
		...
		// 하단 중간에 보이는 페이지 링크 개수
		this.pageCount = 10;
		
		// 게시물 보여 주는 페이지(예시 : boList)
		this.url = url;
		...
		// 현재 페이지에 보여지는 시작 랭킹 번호 
		beginRow = (pageNumber - 1) * pageSize + 1 ;
		
		// 현재 페이지에 보여지는 끝 랭킹 번호 
		...
		// 상단 우측의 현재 페이지 현황(예시 : 총 295건[12/30])
		this.pagingStatus = "총" + totalCount + "건 [" + pageNumber + "/" + totalPage + "]";
		
		// 페이지 이동시 같이 수반되는 파라미터 리스트
		this.flowParameter = "" ;
		...
		this.flowParameter += "&keyword=" + keyword ;
		 ...
		 // 하단의 이전/다음/숫자 목록 페이지 하이퍼 링크를 저장하고 있는 문자열
